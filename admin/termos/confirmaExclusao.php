...
$consulta = $conn->query("select * from usuarios where idusuarios = $id");

while ($usuario = $consulta->fetch(PDO::FETCH_ASSOC)) {  
    $user = $usuario['nome'];
    $sobrenome = $usuario['sobrenome'];
    $id = $usuario['idusuarios'];
    $caminho = $usuario['caminho_download'];
}
...
echo '</br>';
echo '<h4>Deseja realmente excluir o termo de <b>'.$user.' '.$sobrenome.' ? </b></h4>';
...
echo '<a class="btn btn btn-danger" href="excluir_termo.php">Não</a>';
echo '</form>';
echo '</div>';
