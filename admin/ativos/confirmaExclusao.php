...
$consulta = $conn->query("select * from ativos where idativos = $id");

while ($ativos = $consulta->fetch(PDO::FETCH_ASSOC)) {  
    $id = $ativos['idativos'];
}
...
echo '</br>';
echo '<h4>Deseja realmente excluir este ativo ? <b></b></h4>';
echo '<labe>ID: </label><input type="text" name="idativos" id="idativos" value='.$id.' size="1" readonly="readonly">';
echo '</br>';
